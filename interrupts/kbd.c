		...
		size_t size, loff_t *offset)
{
	...
	char ch; bool success; unsigned long flags;

	while (size--) {
		spin_lock_irqsave(&data->lock, flags);
		success = get_char(&ch, data);
		spin_unlock_irqrestore(&data->lock, flags);

		if (!success)
			break;
		
		if (put_user(ch, user_buffer + *offset))
			return -EFAULT;
...
static int kbd_init(void)
{
	...
	if (!request_region(I8042_STATUS_REG + 1, 1, MODULE_NAME)) {
		err = -EBUSY;
		pr_err("I/O port access request failed: %d\n", err);
		goto out_unregister;
	}
